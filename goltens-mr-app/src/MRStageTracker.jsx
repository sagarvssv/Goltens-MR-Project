 ...
 * When status is ISSUED, ALL stages show green tick marks.
 */
export default function MRStageTracker({ status }) {
  const stages = [
    { key: "PENDING",     label: "Manager\nReview"  },
  ...
  const isRejected = status === "REJECTED" || status === "REVERTED";
  const isFullyIssued = status === "ISSUED";
  ...
  return (
    <div style={t.wrap}>
      ...
      {isRejected ? (
        ...
        <div style={t.track}>
          {stages.map((stage, i) => {
            // When ISSUED — every stage is green tick
            const done    = isFullyIssued || currentIdx > i;
            const active  = !isFullyIssued && currentIdx === i;
            const pending = !isFullyIssued && !done && !active;
            ...
            return (
              <div key={stage.key} style={t.step}>
                <div style={{
                  ...
                  ...(active  ? t.dotActive  : {}),
                  ...(done    ? t.dotDone    : {}),
                  ...(pending ? t.dotPending : {}),
                }}>
                  {done ? "✓" : active ? i + 1 : i + 1}
                </div>
                <div style={{
                  ...t.stepLabel,
                  ...(active  ? t.stepLabelActive  : {}),
                  ...(done    ? t.stepLabelDone    : {}),
                  ...(pending ? t.stepLabelPending : {}),
                }}>
                  {stage.label}
                </div>
                {i < stages.length - 1 && (
...
const t = {
  ...
  stepLabel:        { fontSize:10, textAlign:"center", marginTop:5, maxWidth:80, whiteSpace:"pre-line" },
